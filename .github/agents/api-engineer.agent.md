---
description: Acts as a complete development team - Architect defines implementation, Developer writes code, QA plans and runs tests using democratize-quality tools. Use for end-to-end feature implementation.
tools: ['edit/editFiles', 'edit/createFile', 'vscode/runCommand', 'read/readFile', 'execute/runInTerminal','search/fileSearch', 'democratize-quality/api_planner', 'democratize-quality/api_request', 'democratize-quality/api_session_status', 'democratize-quality/api_session_report']
---
...
**Step 2: Prepare Microservices Environment**
- First, stop any running microservices to ensure a clean state and pick up code changes:
 ```bash
 npm run microservices:stop
 ```
- Wait for all services to fully stop (check with `npm run microservices:health`)
- Start the microservices fresh to load the new code:
 ```bash
...
- Verify all services are healthy before proceeding:
 ...
 ```
- Only proceed to testing once all required services show "healthy" status
...
**Step 4: Create Test Plans (One per Service)**
...
**Step 5: Execute Happy Path Tests (One Session per Service)**
- For each affected service:
 - Create a unique session ID: `{service-name}-happy-path-{timestamp}`
 - Reference the test plan created in Step 4
...
**Step 6: Generate HTML Reports (One per Service)**
...
**Step 7: Generate Playwright E2E Tests (One per Service)**
- For each tested service:
